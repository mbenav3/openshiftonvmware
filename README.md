...
1. Run the playbook below to configure your OpenShift cluster to use the nfs server you configured in the steps above

    ```console
    ansible-playbook tasks/nfs-ocp-setup.yml
    ```

2. Create a PVC for the registry using the sample pvc yaml file.

    ```console
    oc create -f /usr/local/src/nfs-registry-pvc.yaml -n openshift-image-registry
    ```

3. Test the auto-provisioner
...
Following the deploymemt of your OpenShift cluster and as you move into deploying and running applications on your cluster, like the Maximo Application Suite, you will need to configure your cluster to store data on different NFS shared directories.

I have provided you with a utility script to help you with applying these configuration changes to your cluster: `/usr/local/bin/nfs-provisioner-setup.sh`

To use this script, you will need to provide the script with 3 pieces of information:

- **nfssharedir** - the name of the 
- **nfsprovisionername** - the name of the 

registry --ocpns nfs-provisioner --nfsprovisionername nfs-storage
